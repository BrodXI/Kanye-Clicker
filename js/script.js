...
function buyRecordClicker() {
  if ( cookies >= arcCost  ) { // todo - remove after set up of unlockUpgrades
    cookies -= arcCost
...
function upgradeARC() {
  if ( cookies >= arcUpgradeCost ) { // todo - remove after set up of unlockUpgrades
...
function arcRun() { // ! for ARC auto clicking function
  if ( !arcState ) {
...
function arcStop() { // ! this is basically just for debugging (stops ARCs)
  if ( arcState ) {
